...
class ABTestManager:
    ...
    def assign_variant(self, contact: str, experiment_id: str) -> Optional[Variant]:
        ...
        if contact in self.user_assignments and experiment_id in self.user_assignments[contact]:
    ...
    def _calculate_winner(self, experiment: ABExperiment) -> Optional[Variant]:
        ...
        if experiment.success_metric == "satisfaction":
            sorted_variants = sorted(experiment.variants, key=lambda v: v.avg_satisfaction_score, reverse=True)
        elif experiment.success_metric == "objectives":
    ...
    def get_experiment_report(self, experiment_id: str) -> Optional[dict[str, Any]]:
        ...
        for variant in experiment.variants:
            success_rate = (
                (variant.successful_conversations / variant.total_conversations * 100)
                if variant.total_conversations > 0
                else 0
            )
            objective_rate = (
                (variant.objectives_achieved / variant.total_conversations * 100) if variant.total_conversations > 0 else 0
            )
            suspicion_rate = (
                (variant.bot_suspicions / variant.total_conversations * 100) if variant.total_conversations > 0 else 0
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "experiments_with_significance": sum(1 for e in self.experiments.values() if e.is_statistically_significant),
        }
