...
class AppointmentFlowManager:
    ...
    async def process_message(self, chat_id: str, message: str, client_phone: str = None) -> tuple[str, bool]:
        """
    ...
    async def _process_name(self, session: AppointmentSession, message: str) -> tuple[str, bool]:
        """Process name input"""
        name = message.strip()
        ...
        if os.getenv("APPOINTMENT_SKIP_EMAIL", "false").lower() == "true":
            ...
            return (f"Gracias {name.split()[0]} 👋\n\n¿Cuál es el motivo de tu cita? (ej: consulta, asesoría, etc.)", False)

        session.state = AppointmentState.COLLECTING_EMAIL
        return (f"Gracias {name.split()[0]} 👋\n\n¿Cuál es tu correo electrónico? (para enviarte la confirmación)", False)

    async def _process_email(self, session: AppointmentSession, message: str) -> tuple[str, bool]:
        """Process email input"""
        ...
        if not email_match:
            ...
            if any(word in message.lower() for word in ["no tengo", "no", "skip", "saltar", "omitir"]):
                session.state = AppointmentState.COLLECTING_REASON
                return ("Sin problema. ¿Cuál es el motivo de tu cita?", False)

            session.retry_count += 1
            if session.retry_count >= session.max_retries:
                session.state = AppointmentState.COLLECTING_REASON
                return ("Continuemos sin email. ¿Cuál es el motivo de tu cita?", False)
            return ("No pude identificar un email válido. Por favor escríbelo nuevamente (o escribe 'no' para omitir):", False)

        session.client_email = email_match.group()
        session.retry_count = 0
        session.state = AppointmentState.COLLECTING_REASON

        return ("Perfecto ✅\n\n¿Cuál es el motivo de tu cita? (ej: consulta, asesoría, reunión)", False)

    async def _process_phone(self, session: AppointmentSession, message: str) -> tuple[str, bool]:
        ...
        session.state = AppointmentState.COLLECTING_REASON
        return ("¿Cuál es el motivo de tu cita?", False)

    async def _process_reason(self, session: AppointmentSession, message: str) -> tuple[str, bool]:
    ...
    async def _process_date(self, session: AppointmentSession, message: str) -> tuple[str, bool]:
    ...
    async def _show_generic_slots(self, session: AppointmentSession) -> tuple[str, bool]:
    ...
    async def _show_calendar_slots(self, session: AppointmentSession) -> tuple[str, bool]:
    ...
    def _format_slots_message(self, session: AppointmentSession) -> tuple[str, bool]:
    ...
    async def _process_slot_selection(self, session: AppointmentSession, message: str) -> tuple[str, bool]:
    ...
    async def _process_confirmation(self, session: AppointmentSession, message: str) -> tuple[str, bool]:
        ...
        if any(word in message_lower for word in ["no", "cancelar", "cancel", "cambiar"]):
            self.end_session(session.chat_id)
            return (
                "Entendido, he cancelado la reserva. Puedes escribir 'agendar cita' cuando quieras intentar de nuevo.",
                True,
            )

        return ("Por favor confirma con 'sí' o cancela con 'no':", False)

    async def _create_appointment(self, session: AppointmentSession) -> tuple[str, bool]:
        ...
        if self._calendar.is_ready():
            try:
                ...
                if result.success:
                    ...
                    response = f"✅ *¡Cita confirmada!*\n\n📅 {slot.format_for_user()}\n"
