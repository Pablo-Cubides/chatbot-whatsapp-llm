...
class GoogleCalendarProvider(BaseCalendarProvider):
        ...
        try:
            ...
            if credentials:
                if credentials.get("type") == "service_account":
                    creds = ServiceAccountCredentials.from_service_account_info(credentials, scopes=SCOPES)
                else:
                    creds = Credentials(
    ...
    async def create_appointment(self, appointment: AppointmentData) -> AppointmentResult:
        ...
        if not self.is_authenticated or not self._service:
            return AppointmentResult(
                success=False, error_message="Not authenticated with Google Calendar", provider=self.provider_name
            )
        ...
        try:
            ...
            event = {
                ...
                "start": {"dateTime": appointment.start_time.isoformat(), "timeZone": appointment.timezone},
                "end": {"dateTime": appointment.end_time.isoformat(), "timeZone": appointment.timezone},
                "attendees": [],
                "reminders": {
                    "useDefault": False,
                    "overrides": [
            ...
            if appointment.client_email:
                event["attendees"].append({"email": appointment.client_email, "displayName": appointment.client_name})
            ...
            return AppointmentResult(success=False, error_message=error_msg, provider=self.provider_name)
        except Exception as e:
    ...
    async def update_appointment(self, external_id: str, updates: dict[str, Any]) -> AppointmentResult:
        ...
        try:
            # Get existing event
            event = self._service.events().get(calendarId=self.config.calendar_id, eventId=external_id).execute()
            ...
            updated_event = (
                self._service.events()
                .update(calendarId=self.config.calendar_id, eventId=external_id, body=event, sendUpdates="all")
                .execute()
            )
            ...
            return AppointmentResult(success=False, error_message=f"API error: {e.reason}", provider=self.provider_name)
        except Exception as e:
            return AppointmentResult(success=False, error_message=str(e), provider=self.provider_name)
    ...
    async def cancel_appointment(self, external_id: str, send_notification: bool = True) -> bool:
        ...
        try:
            self._service.events().delete(
                calendarId=self.config.calendar_id, eventId=external_id, sendUpdates="all" if send_notification else "none"
            ).execute()
