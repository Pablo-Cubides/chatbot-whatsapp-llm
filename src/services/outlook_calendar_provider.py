...
class OutlookCalendarProvider(BaseCalendarProvider):
    ...
    def _save_token_cache(self):
        ...
        if self._msal_app and self._msal_app.token_cache.has_state_changed:
            ...
            self._token_cache_path.write_text(self._msal_app.token_cache.serialize())
    ...
    async def _verify_token(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{GRAPH_API_BASE}/me", headers={"Authorization": f"Bearer {self._access_token}"})
                return response.status_code == 200
        except Exception:
            return False

    def get_oauth_url(self, redirect_uri: str = "http://localhost:8003/api/calendar/oauth/outlook/callback") -> Optional[str]:
        """
        ...
        try:
            authorization_url = app.get_authorization_request_url(
                scopes=SCOPES, redirect_uri=redirect_uri, state=str(uuid.uuid4())
            )
    ...
    async def handle_oauth_callback(self, authorization_code: str, redirect_uri: str) -> bool:
        ...
        try:
            result = app.acquire_token_by_authorization_code(code=authorization_code, scopes=SCOPES, redirect_uri=redirect_uri)
    ...
    async def get_free_slots(self, start_date: datetime, end_date: datetime, duration_minutes: int = 30) -> list[TimeSlot]:
        ...
        try:
            ...
            endpoint = (
                f"/me/calendars/{calendar_id}/calendarView?startDateTime={start_iso}&endDateTime={end_iso}&$select=start,end"
            )
            ...
            while current_date < end_date:
                day_slots = self._generate_slots_from_working_hours(current_date, busy_periods, duration_minutes)
    ...
    async def create_appointment(self, appointment: AppointmentData) -> AppointmentResult:
        ...
        if not self.is_authenticated:
            return AppointmentResult(
                success=False, error_message="Not authenticated with Outlook", provider=self.provider_name
            )
        ...
        try:
            ...
            event = {
                ...
                "body": {"contentType": "HTML", "content": self._build_description_html(appointment)},
                "start": {"dateTime": appointment.start_time.strftime("%Y-%m-%dT%H:%M:%S"), "timeZone": appointment.timezone},
                "end": {"dateTime": appointment.end_time.strftime("%Y-%m-%dT%H:%M:%S"), "timeZone": appointment.timezone},
                "isReminderOn": True,
            ...
            if appointment.location:
                event["location"] = {"displayName": appointment.location}

            # Add attendee
            if appointment.client_email:
                event["attendees"] = [
                    {
                        "emailAddress": {"address": appointment.client_email, "name": appointment.client_name},
                        "type": "required",
                    }
                ]
            ...
            if not result:
                return AppointmentResult(success=False, error_message="Failed to create event", provider=self.provider_name)
            ...
            return AppointmentResult(success=False, error_message=str(e), provider=self.provider_name)
    ...
    async def update_appointment(self, external_id: str, updates: dict[str, Any]) -> AppointmentResult:
        ...
        if not self.is_authenticated:
            return AppointmentResult(success=False, error_message="Not authenticated", provider=self.provider_name)
        ...
        try:
            ...
            if "description" in updates:
                event_updates["body"] = {"contentType": "HTML", "content": updates["description"]}
            ...
            if "location" in updates:
                event_updates["location"] = {"displayName": updates["location"]}

            result = await self._make_graph_request("PATCH", f"/me/events/{external_id}", event_updates)

            if result:
                logger.info(f"✅ Updated Outlook event: {external_id}")
                return AppointmentResult(
                    success=True, external_id=external_id, external_link=result.get("webLink"), provider=self.provider_name
                )

            return AppointmentResult(success=False, error_message="Update failed", provider=self.provider_name)
    ...
    async def cancel_appointment(self, external_id: str, send_notification: bool = True) -> bool:
        ...
        try:
            result = await self._make_graph_request("DELETE", f"/me/events/{external_id}")
    ...
    async def get_appointment(self, external_id: str) -> Optional[dict[str, Any]]:
        ...
        try:
            result = await self._make_graph_request("GET", f"/me/events/{external_id}")
