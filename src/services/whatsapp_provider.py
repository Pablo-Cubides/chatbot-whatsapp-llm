...
class DualProvider(WhatsAppProvider):
    ...
    def __init__(self):
        ...
        self.use_backup_for = []  # Lista de chat_ids que deben usar backup
    ...
    def send_message(
        ...
        if self.primary.is_available():
            ...
            # Si falla, marcar para usar backup
            logger.warning(f"⚠️ Cloud API falló para {chat_id}, usando Web backup")
            self.use_backup_for.append(chat_id)
...
def get_provider() -> WhatsAppProvider:
    ...
    if _current_provider is None:
        ...
        if mode == "both":
            _current_provider = DualProvider()
        else:
            _current_provider = WhatsAppProviderFactory.create_from_env()
