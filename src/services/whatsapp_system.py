...
try:
    from src.services.humanized_responses import response_manager  # noqa: F401
    from src.services.silent_transfer import silent_transfer_manager  # noqa: F401
...
class WhatsAppManager:
    ...
    async def _process_incoming_message(self):
        ...
        try:
            ...
            if contact_name not in self.active_chats:
                ...
                if self.analytics:
                    self.analytics.record_conversation_start(self.active_chats[contact_name]["session_id"], contact_name)
            ...
            if APPOINTMENT_FLOW_AVAILABLE and appointment_flow:
                ...
                appointment_response, flow_completed = await appointment_flow.process_message(
                    chat_id=contact_name, message=message_text, client_phone=client_phone
                )
    ...
    async def _detect_and_download_image(self) -> Optional[bytes]:
        ...
        try:
            ...
            image_element = await self.page.query_selector('[data-testid="msg-container"]:last-child img[src^="blob:"]')
    ...
    async def _generate_response(self, contact_name: str, message_text: str) -> Optional[str]:
        ...
        try:
            ...
            context_messages = [{"role": "system", "content": self.business_config._build_main_prompt(config)}]
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "uptime_seconds": (
                datetime.now() - min((chat["started_at"] for chat in self.active_chats.values()), default=datetime.now())
            ).total_seconds()
